...
fore_train_op = joblib.load(open('fore_train_op.pkl', 'rb'))
fore_valid_op = joblib.load(open('fore_valid_op.pkl', 'rb'))
fore_train_ip = joblib.load(open('fore_train_ip.pkl', 'rb'))
fore_valid_ip = joblib.load(open('fore_valid_ip.pkl', 'rb'))

d_model, e_layers, d_layers, d_ff = 256, 2, 2, 2048
number_of_epochs, batch_size = 100, 16
V, D = 98, 17
fore_max_len = 2640
...
class StratsModel(nn.Module):
    ...
    def forward(self, demo, times, values, varis, trainn=False, tgt=torch.zeros(batch_size, 1, V).cuda()):
        ...
        if self.autoregressive:
            if self.trainn:
                ...
                start_input = torch.zeros(batch_size, 1, V).cuda()
                tgt = torch.concat([start_input, tgt], dim=1)
                result=self.dec(tgt, linear_memory, mask_new)
                return result[:, 1:, :]
            if not self.trainn:
                output = torch.zeros(batch_size, 1, V).cuda()
                linear_memory  = self.linearize(cont_emb)
                for i in range(24):
                    #print(i)
...
lr, samples_per_epoch, patience = 0.0005, 102400, 6
...
#print(N_fore)
fore_savepath = 'eicu_mod02_Triplet_IMS_TF.pytorch' # before 'eicu_24h_strats_no_interp_with_ss_fore_strats_ims_tf.pytorch'
...
for e in range(number_of_epochs):
    ...
    for start in pbar:
        ...
        output = model(*[torch.tensor(ip[ind], dtype=torch.float32).cuda() if i < 3 
                         ...
                         for i, ip in enumerate(fore_train_ip)], trainn=True, tgt=torch.tensor(fore_train_op[ind, :, :V], dtype=torch.float32).cuda())
        #raise Exception
        #print(fore_train_op[ind].shape)
        #raise Exception
        loss = torch.tensor(fore_train_op[ind, :, V:], dtype=torch.float32).cuda()*(
            output-torch.tensor(fore_train_op[ind, :, :V], dtype=torch.float32).cuda())**2
    ...
    entry = 'Model 02 Epoch ' +str(e) + ' loss ' + str(loss_p.item()) + ' val loss ' + str(val_loss_p.item()) + " mean " + str(np.mean(loss_list)) +' std '+ str(np.std(loss_list)) + '\n'
