...
import models.DenseDMSFull as autoformer
...
#print(N_fore)
fore_savepath = 'eicu_mod07_Dense_DMS.pytorch'
...
for e in range(number_of_epochs):
    ...
    for start in pbar:
        ...
        if 'Linear' not in str(type(model)) and "Autoregressive" in str(type(model)):
            output = model(input_matrix, input_mark, dec_inp, output_mark, tgt=output_matrix, trainn=False, backprop=False)#, enc_self_mask=input_mask, dec_self_mask=output_mask)
    ...
    entry = 'Model 07 Epoch ' +str(e) + ' loss ' + str(loss_p.item()) + ' val loss ' + str(val_loss_p.item()) + " mean " + str(np.mean(loss_list)) +' std '+ str(np.std(loss_list)) + '\n'
