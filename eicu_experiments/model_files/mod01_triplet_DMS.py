...
fore_savepath = 'eicu_mod01_Triplet_DMS.pytorch' # was wrongly called mod04
...
for e in range(number_of_epochs):
    ...
    model.eval()
    loss_list = []
    pbar = tqdm(range(0, len(fore_valid_op)-batch_size, batch_size))
    for start in pbar:
    ...
    entry = 'Model 01 Epoch ' +str(e) + ' loss ' + str(loss_p.item()) + ' val loss ' + str(val_loss_p.item()) + " mean " + str(np.mean(loss_list)) +' std '+ str(np.std(loss_list)) + '\n'
