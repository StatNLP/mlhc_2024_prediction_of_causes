...
#print(N_fore)
fore_savepath = 'eicu_mod04_Triplet_IMS_SF_BP.pytorch'
...
for e in range(number_of_epochs):
    ...
    for start in pbar:
        ...
        output = model(*[torch.tensor(ip[ind], dtype=torch.float32).cuda() if i < 3 
                         ...
                         for i, ip in enumerate(fore_train_ip)], trainn=False, tgt=torch.tensor(fore_train_op[ind, :, :V], dtype=torch.float32).cuda())
        #raise Exception
    ...
    entry = 'Model 04 Epoch ' +str(e) + ' loss ' + str(loss_p.item()) + ' val loss ' + str(val_loss_p.item()) + " mean " + str(np.mean(loss_list)) +' std '+ str(np.std(loss_list)) + '\n'
