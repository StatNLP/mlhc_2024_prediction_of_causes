...
fore_savepath = 'eicu_autoformer.pytorch'
...
for e in range(number_of_epochs):
    ...
    for start in pbar:
        ...
        if 'Linear' not in str(type(model)) and "Autoregressive" in str(type(model)):
            output = model(input_matrix, input_mark, dec_inp, output_mark, tgt=output_matrix, trainn=False, backprop=False)
        elif "Autoformer" in str(type(model)):
