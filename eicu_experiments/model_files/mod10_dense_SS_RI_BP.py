...
sample_divisor = 1
number_of_epochs,V,D = 1000,98,17
fore_max_len = 2640
...
# %%
import models.DenseAutoregressiveSchedule as autoformer
...
for e in range(number_of_epochs):
    ...
    for start in pbar:
        ...
        if 'Linear' not in str(type(model)) and "Autoregressive" in str(type(model)):
            output = model(input_matrix, input_mark, dec_inp, output_mark, tgt=output_matrix, trainn=True, backprop=True, schedule=True, deterministic=False, probability=prob)#, enc_self_mask=input_mask, dec_self_mask=output_mask)
