...
#print(N_fore)
fore_savepath = 'eicu_mod08_Dense_IMS_TF.pytorch'
...
for e in range(number_of_epochs):
    ...
    for start in pbar:
        ...
        if 'Linear' not in str(type(model)) and "Autoregressive" in str(type(model)):
            output = model(input_matrix, input_mark, dec_inp, output_mark, tgt=output_matrix, trainn=True, backprop=False)#, enc_self_mask=input_mask, dec_self_mask=output_mask)
